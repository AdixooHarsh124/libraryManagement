...
public class subscriberController {
	...
	{
		book books=rest.getForObject("http://book-microservice/library/books/"+subcriber.getIssueBookId(),book.class);
		...
		if(books.getRemainingBooks() != 0 && books.getTotelNumberOfCopies() >= books.getRemainingBooks()) 
		{
			...
			rest.put("http://book-microservice/library/books/"+subcriber.getIssueBookId(), books);
	...
	public ResponseEntity<subscriberModel> removeSubscriber(@PathVariable("subscriberId") int subscriberId)
	{
		...
		try {
			...
			book book=this.rest.getForObject("http://book-microservice/library/books/"+ subscriberDetails.getIssueBookId(),book.class);
			
			int availableBooks=book.getRemainingBooks();
			availableBooks++;
			
			book.setRemainingBooks(availableBooks);
			rest.put("http://book-microservice/library/books/"+ subscriberDetails.getIssueBookId(), book);
	...
	public void updateUserDetails(@RequestBody subscriberModel subscriber,@PathVariable("subscriberId") int subscriberId)
	{
	   ...
	   userDetails.setIssueBookId(subscriber.getIssueBookId());
